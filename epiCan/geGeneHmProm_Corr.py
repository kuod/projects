...
resultsFolder = '/cbio/grlab/home/dkuo/projects/epican/results/131210/'
...
for cellType in cellTypeList:
    ...
    for geneFile in geneFiles:
        if geneFile.endswith(cellType + ".hdf5"):
            try:
                ...
                #star position begins 500 bp upstream
                cellTypeHmVec.append(np.sum(g[0:750,hmIdx[0]]))
                f.close()
            except(IOError):
    ...
    ax.set_ylabel('H3k4me3 across prom (log fpkm)')
    #ax.set_ylim(0,2E6)
    ax.set_title(cellType + ': Correlation of ' + str(cellTypeCorr))
    figName = resultsFolder + cellType + '_expGeGene_HmProm_corr.png'
