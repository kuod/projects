...
outputDir = '/cbio/grlab/share/databases/encode/hdf5_2kb/'
...
def saveCellTypeDat(genes, chromosomes, starts, stops, cellType, geFileList, hmFileList):
    start = time.time()
        ...
        if os.path.isfile(fileName):
            ...
            for ge_count in range(len(geFileList)):
                if ge_count == 0:
                    ...
                    oneGeMat = np.mat(geFile.get_as_array(chromosomes[i], starts[i], stops[i]).T)
            ...
            for hm_count in range(len(hmFileList)):
                if hm_count == 0:
                    ...
                    oneHmMat = np.mat(hmFile.get_as_array(chromosomes[i], starts[i], stops[i]).T)
...
for ct in cellTypeList:
    ...
    saveCellTypeDat(geneSubset, chrSubset, startSubset, stopSubset, ct, geFileList, hmFileList)
